	...
	static {
		try {
			...
			*****************************************************/
			...
			// 위의 과정을 체이닝 형태로 한 번에 처리한게 아래 형태
			factory = new SqlSessionFactoryBuilder().build(Resources.getResourceAsReader("bit/second/mybatis/config.xml"));
			
			
			
			// Resouces.getResouceAsReader - config 파일을 읽어오는 역할
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
